/**
 * E2E test for `jorvel ssr export` — static HTML export.
 *
 * This test is skipped unless JORVEL_E2E=1 is set (same gate as other e2e tests).
 * It:
 *   1. Creates a temporary workspace with a minimal App + template.
 *   2. Calls `staticExport` directly (the same code `jorvel ssr export` invokes).
...
<html lang="en">
<head><meta charset="UTF-8" /><title>JORVEL Static</title></head>
...
async function makeTmp() {
  return fs.mkdtemp(path.join(os.tmpdir(), 'jorvel-ssr-e2e-'));
}
...
test('static export — generates index.html for root "/"', async () => {
  const { staticExport } = await import('@jorvel/ssr');
...
test('streaming SSR — renders full HTML via stream with correct content', async () => {
  const { renderRouteToStream, collectStream } = await import('@jorvel/ssr');
...
test('edge adapter — handles request and returns correct HTML', async () => {
  const { createEdgeAdapter } = await import('@jorvel/ssr');
...
test('server router — createServerRouter is safe in Node.js (no window)', async () => {
  const { createServerRouter } = await import('@jorvel/runtime');
