...
test('@build remote mfjs.federation.json exposes "./App" container', async () => {
  const cfg = JSON.parse(
    await fs.readFile(path.join(exampleRoot, 'apps', 'dashboard', 'mfjs.federation.json'), 'utf8')
  );
...
test('@build shell mfjs.federation.json references dashboard remoteEntry.js', async () => {
  const cfg = JSON.parse(
    await fs.readFile(path.join(exampleRoot, 'apps', 'shell', 'mfjs.federation.json'), 'utf8')
  );
...
test('@build remote dist does not bundle a duplicate @mfjs/event-bus (singleton sharing)', async () => {
