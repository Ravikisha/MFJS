export const JORVEL_REMOTE_LOAD_EVENT = 'jorvel:remote-load';
export const JORVEL_ERROR_EVENT = 'jorvel:error';
...
export function emitRemoteLoad(detail: RemoteLoadTelemetryDetail): void {
  ...
  window.dispatchEvent(new CustomEvent(JORVEL_REMOTE_LOAD_EVENT, { detail }));
}

export function emitError(detail: ErrorTelemetryDetail): void {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent(JORVEL_ERROR_EVENT, { detail }));
}
...
export function onRemoteLoad(cb: (d: RemoteLoadTelemetryDetail) => void): () => void {
  ...
  return () => window.removeEventListener(JORVEL_REMOTE_LOAD_EVENT, handler);
}
...
export function onRuntimeError(cb: (d: ErrorTelemetryDetail) => void): () => void {
  ...
  window.addEventListener(JORVEL_ERROR_EVENT, handler);
  return () => window.removeEventListener(JORVEL_ERROR_EVENT, handler);
}
