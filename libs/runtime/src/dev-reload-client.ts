type DevReloadMessage =
  | { type: 'mfjs:reload'; reason?: string }
  | { type: 'mfjs:ping' };
...
/**
 * Connects to the MFJS dev reload server (started by `mfjs dev --hmr-remotes`).
 *
 ...
 */
export function connectMfjsDevReload(options?: {
  url?: string;
  onReload?: (reason?: string) => void;
}) {
  const url = options?.url || (globalThis as any).__MFJS_DEV_RELOAD_URL__;
  ...
  const onReload =
    ...
    ((reason?: string) => {
      ...
      console.log('[mfjs] reload requested', reason || '');
  ...
  const connect = () => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (!msg) return;
      if (msg.type === 'mfjs:reload') onReload(msg.reason);
    };
