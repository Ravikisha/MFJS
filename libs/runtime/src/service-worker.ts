export interface ServiceWorkerRegisterOptions {
  /** URL to the SW script. Default: '/jorvel-sw.js'. */
  url?: string;
  /** Scope. Default: '/'. */
  scope?: string;
...
export async function registerJorvelServiceWorker(
  ...
  const url = opts.url ?? '/jorvel-sw.js';
  ...
  try {
    ...
    console.warn('[jorvel] service worker registration failed', err);
    return null;
  }
}

export async function unregisterJorvelServiceWorker(): Promise<boolean> {
...
/** Inline SW script source — bundlers can emit via `import sw from './jorvel-sw-source?raw'`. */
export const JORVEL_SERVICE_WORKER_SOURCE = `/* generated by @jorvel/runtime */
