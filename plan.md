# JORVEL Production Readiness Plan

Audit of current JORVEL micro-frontend framework vs production-grade targets (Next.js / Nx parity). Lists what exists, what gaps remain, prioritized roadmap to ship.
...
- Type-safe federation contracts via `@jorvel/types` (`InferExposed` / `InferEmits` / `InferListens`).
...
- **Action:** ship `@jorvel/security` helpers. CSP builder, SRI hash generator at build, allowlist config in `jorvel.config.ts`, iframe-sandbox remote option, OAuth/session propagation example + helper hook.
...
- **Action:** `@jorvel/observability` package. `onError(fn)`, `onMetric(fn)`, `onRemoteLoad(fn)` hooks. Adapter stubs for Sentry, OTEL. Web Vitals collector. Include in runtime default (noop unless wired).
...
- No schema validation for `jorvel.federation.json` + `jorvel.app.json`.
- **Action:** `RemoteRegistry` singleton with runtime `register(name, url)`. Version compare + warn in `loadRemoteEntry`. CSS module / scoped CSS generator option. `publicPath` field in `jorvel.config.ts`. `jorvel build --stats` JSON output. Zod schemas published to `https://jorvel.dev/schemas/*`.
...
- **Action:** adapter packages `@jorvel/adapter-vercel`, `@jorvel/adapter-cloudflare`, `@jorvel/adapter-node`, `@jorvel/adapter-docker`. `jorvel deploy --target=<adapter>`. `jorvel env check` validates required vars.
...
### 8. CLI Enhancements
- Missing: `jorvel lint`, `jorvel test`, `jorvel deploy`, `jorvel routes --watch`, `jorvel diagnose`, `--dry-run`, `--verbose`.
- Partial: `jorvel image`, `jorvel lazy` (stubbed only).
...
- No `@jorvel/eslint-config`, `@jorvel/prettier-config`, `@jorvel/tsconfig`, `@jorvel/tailwind-preset`.
...
### 12. i18n
- None shipped.
- **Action:** `@jorvel/i18n` wrapper around `formatjs` or `i18next`. Per-remote message catalogs. SSR locale detection via `Accept-Language`.
...
- E2E gated behind `JORVEL_E2E=1` — CI may skip.
...
### 14. Performance
- `jorvel perf` budget-check exists but no bundle analyzer.
- No lighthouse CI integration.
- No automatic image optimization (command stub only).
- No historical perf tracking.
- **Action:** wire `rspack-bundle-analyzer`. Lighthouse GH Action in scaffolded CI. Implement `jorvel image` via `sharp`. Output `perf.json` artifact per build for trending.
...
- Schema URL `https://jorvel.dev/schemas/...` referenced but not published.
...
- **Dynamic remote discovery**: registry service + `jorvel.config.ts` `discover: { url }`. Remotes self-register at deploy time.
- **Visual route editor**: web UI → flowchart → generates `jorvel.routes.host.json`.
- **Perf dashboard during dev**: live size/load metrics per remote in `jorvel dev` output.
...
2. Schema publish (`jorvel.config`, `jorvel.app`, `jorvel.federation`).
3. `CHANGELOG.md` per package. Git tags.
4. Fix `0.0.0` versioning everywhere.
5. Shared `@jorvel/eslint-config`, `@jorvel/tsconfig`, `@jorvel/prettier-config`.

### Phase 1 — Production Minimums (4 weeks)
1. Security: CSP builder + SRI + remote allowlist + auth hook.
2. Observability: `@jorvel/observability` + error/metric/remote-load hooks + Sentry adapter.
...
1. Design system: real `@jorvel/ui` components + Storybook + Tailwind preset.
2. State upgrades: middleware, selectors, persistence, SSR hydrate, devtools.
3. i18n package.
4. Contract tests + visual regression + a11y.
5. Bundle analyzer + Lighthouse CI + `jorvel image` via sharp.
...
|---|---|---|
| `@jorvel/eslint-config` | Shared lint rules | 0 |
| `@jorvel/tsconfig` | Shared TS base | 0 |
| `@jorvel/prettier-config` | Shared formatter | 0 |
| `@jorvel/security` | CSP, SRI, allowlist | 1 |
| `@jorvel/observability` | Error/metric/telemetry hooks | 1 |
| `@jorvel/adapter-vercel` | Vercel deploy | 1 |
| `@jorvel/adapter-cloudflare` | CF Pages + Workers | 1 |
| `@jorvel/adapter-node` | Node.js server | 1 |
| `@jorvel/adapter-docker` | Dockerfile scaffold | 1 |
| `@jorvel/tailwind-preset` | Design tokens | 2 |
| `@jorvel/i18n` | Translations + SSR | 2 |
| `@jorvel/devtools` | Redux DevTools bridge | 2 |
| `@jorvel/discovery` | Runtime remote registry | 3 |
...
4. Create `@jorvel/eslint-config`, `@jorvel/tsconfig` packages. Migrate repo apps.
5. Publish JSON schemas referenced by generated `jorvel.config.ts`.
