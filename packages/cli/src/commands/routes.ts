...
// NOTE: we keep the implementation local to @moxjs/cli to avoid TS rootDir
// cross-package source imports during compilation.
// The *contract* lives in @moxjs/types (libs/types/src/routing-compiler.ts).
type MoxjsPageRoute = { path: string; file: string };
...
const defaultRoutingCompiler = {
  ...
  sortRoutesForMatching(routes: MoxjsPageRoute[]) {
...
function sortRoutesForMatching(routes: PageRoute[]) {
  return defaultRoutingCompiler.sortRoutesForMatching(routes as MoxjsPageRoute[]) as PageRoute[];
}

async function writeRemoteRoutesModule(appDir: string, routes: PageRoute[]) {
  const outFile = path.join(appDir, 'src', 'moxjs.routes.ts');
  const lines: string[] = [];

  lines.push('// THIS FILE IS AUTO-GENERATED by `moxjs routes`.');
  lines.push('// It exposes the remote\'s file-based route table for the host to consume.');
  lines.push("import type { RemotePageRoute } from '@moxjs/runtime';");
  lines.push('');
  lines.push('export const pages: RemotePageRoute[] = [');

  const sorted = sortRoutesForMatching(routes);
  for (const r of sorted) {
    // Keep the import target relative to this file (src/moxjs.routes.ts).
...
async function generateAllRoutes(workspaceDir: string): Promise<void> {
  ...
  for (const app of apps) {
    ...
    const outPath = path.join(app.dir, 'moxjs.routes.json');
  ...
  if (host) {
    ...
    const outPath = path.join(host.dir, 'moxjs.routes.host.json');
...
export const routesCommand = new Command('routes')
  ...
  .action(async (opts: { dir: string; watch?: boolean }) => {
    ...
    for (const app of apps) {
      ...
      if (app.meta.type === 'remote') {
        await writeRemoteRoutesModule(app.dir, routes);
        console.log(kleur.green(`wrote ${path.relative(workspaceDir, path.join(app.dir, 'src/moxjs.routes.ts'))}`));
      }
    }
    ...
    console.log(kleur.cyan('Done. Next: update your host to load moxjs.routes.host.json and mount remotes based on the URL.'));
  });
