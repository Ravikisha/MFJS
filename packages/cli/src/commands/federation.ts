...
async function detectExposes(appDir: string, meta: AppMeta): Promise<Record<string, string> | undefined> {
  ...
  // Optional convention: file-based routing module generated by mfjs routes
  // Remotes can expose this for the host to render remote pages.
  const routesModule = path.join(appDir, 'src', 'mfjs.routes.ts');
  const hasRoutesModule = await fs.pathExists(routesModule);

  // If exposes are explicitly set in mfjs.app.json, respect them but still allow
  // the auto-generated Routes module to be added.
  if (meta.exposes && Object.keys(meta.exposes).length > 0) {
    const exposes: Record<string, string> = { ...meta.exposes };
    if (hasRoutesModule && !exposes['./Routes']) exposes['./Routes'] = './src/mfjs.routes.ts';
    return exposes;
  }
  ...
  const exposes: Record<string, string> = { './App': './src/remote.tsx' };
  if (hasRoutesModule) exposes['./Routes'] = './src/mfjs.routes.ts';
  return exposes;
  }
  ...
  const exposes: Record<string, string> = { './App': './src/App.tsx' };
  if (hasRoutesModule) exposes['./Routes'] = './src/mfjs.routes.ts';
  return exposes;
  }
