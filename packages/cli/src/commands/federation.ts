...
async function detectExposes(appDir: string, meta: AppMeta): Promise<Record<string, string> | undefined> {
  ...
  if (meta.exposes && Object.keys(meta.exposes).length > 0) return meta.exposes;
...
async function detectSharedFromPackageJson(appDir: string) {
  ...
  const candidates = ['react', 'react-dom', 'react-router-dom', '@mfjs/event-bus', '@mfjs/state', '@mfjs/ui'];
