...
export const initCommand = new Command('init')
  .description('Initialize a new MOXJS workspace (monorepo)')
  ...
  .action(async (name: string, opts: { dir: string; yes?: boolean; tailwind?: boolean }) => {
    ...
    console.log(kleur.cyan(`Creating MOXJS workspace in ${workspaceDir}`));
  ...
  const interactive = !opts.yes && Boolean(process.stdout.isTTY);
    ...
    await writeJson(path.join(workspaceDir, 'package.json'), {
      ...
      scripts: {
        ...
        build: 'pnpm -r build',
        test: 'pnpm -r test',
        typecheck: 'moxjs typecheck',
        'ci:affected': 'moxjs ci affected',
      },
      devDependencies: {
        '@moxjs/types': '^0.1.0',
      },
    ...
    await writeText(
      ...
      `# ${name}\n\nGenerated by @moxjs/cli.\n`
    );

    // moxjs.config.json
    await writeJson(path.join(workspaceDir, 'moxjs.config.json'), {
      $schema: './node_modules/@moxjs/types/schemas/moxjs.config.json',
      appsDir: 'apps',
      libsDir: 'libs',
      features: {
    ...
    // moxjs.config.ts
    await writeText(
      path.join(workspaceDir, 'moxjs.config.ts'),
      [
  ...
  "  libsDir: 'libs',",
  '  features: {',
  `    tailwind: ${enableTailwind ? 'true' : 'false'},`,
  '  },',
  '  orchestrator: {',
  "    mode: 'parallel',",
  '    proxyRemotes: false,',
  ...
  console.log(kleur.gray('  moxjs scaffold app  # guided full app scaffold (host + remotes)'));
  console.log(kleur.gray('  moxjs generate host shell'));
  console.log(kleur.gray('  moxjs generate remote dashboard'));
    console.log(kleur.gray('  moxjs dev'));
  });
