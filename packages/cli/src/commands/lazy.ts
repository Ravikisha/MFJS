...
const DEFAULT_SUSPICIOUS_PATTERNS: Array<{ pattern: string; regex: RegExp; message: string }> = [
  {
    ...
    message:
      'Detected dynamic import of a federation exposed module in built output. Prefer loading remotes via @jorvel/runtime loadRemoteModule in the host runtime so the bundle stays lean.',
  },
  {
    pattern: 'jorvel: eager remote entry reference',
