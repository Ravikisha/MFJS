...
async function writeHostRemoteSmokeTest(workspaceDir: string, hostName: string, remoteNames: string[]) {
  ...
  await fs.outputFile(
    ...
    [
      ...
      "  it('host and remotes exist with moxjs.app.json', async () => {",
      ...
      '      expect(await fs.pathExists(path.join(dir, \"moxjs.app.json\"))).toBe(true);',
      ...
      '    for (const name of apps) {',
      '      const dir = path.join(appsDir, name);',
      '      expect(await fs.pathExists(path.join(dir, \"moxjs.federation.json\"))).toBe(true);',
      '    }',
      '  });',
      '});',
      '',
  ...
  .action(async (opts: ScaffoldOpts) => {
    ...
    const selected = interactive
      ? await checkbox({
          ...
          choices: [
            { name: 'Generate moxjs.routes.json (file-based routes)', value: 'routes', checked: true },
            { name: 'Generate moxjs.federation.json (Module Federation wiring)', value: 'federation', checked: true },
