...
describe('jorvel init', () => {
  ...
  it('writes jorvel.config.json with appsDir and libsDir fields', async () => {
    const tmp = (await fs.mkdtemp(path.join(os.tmpdir(), 'jorvel-init-'))) as string;
    await run('my-app', tmp);
  ...
  it('writes jorvel.config.ts as a TypeScript module', async () => {
    const tmp = (await fs.mkdtemp(path.join(os.tmpdir(), 'jorvel-init-'))) as string;
    await run('my-app', tmp);
    const ts = await fs.readFile(path.join(tmp, 'my-app', 'jorvel.config.ts'), 'utf8');
